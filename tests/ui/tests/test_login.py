...
def test_logout(browser_page, base_url, login):
    ...
    expect(login_page.username_input, "Username input is not visible").to_be_visible(timeout=20000)
    expect(login_page.password_input, "Password input is not visible").to_be_visible(timeout=20000)
    expect(login_page.login_button, "Login button is not visible").to_be_visible(timeout=20000)
