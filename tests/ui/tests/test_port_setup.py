...
def test_open_port_setup_page(browser_page, base_url, login):
    ...
    expect(port_setup.port_setup_content, "Port Setup header is not visible").to_be_visible()
    expect(port_setup.save_changes_btn, "Save Changes button is not visible").to_have_count(1)
...
all_roles = ["inbound only", "full access", "repositioning disabled", "outbound only"]
def test_change_all_roles_and_toggle(browser_page, base_url):
    """Cycle through all roles and toggle states with clear assertion messages."""
    port_page = PortSetupPage(browser_page, base_url)

    for role in all_roles:
        port_page.select_role(role)

        # Get current toggle state for this role
        ...
        assert selected_role == role, (
            f"Role selection failed: expected '{role}', got '{selected_role}'"
        )
        assert toggle_state != current_toggle, (
            f"Trans-shipment toggle did not change for role '{role}': "
            f"expected opposite of {current_toggle}, got {toggle_state}"
        )
